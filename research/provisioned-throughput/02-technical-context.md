...
|---|---|---|---|---|
| H100 NVL | 80 GB HBM3 | Hopper | PCIe + NVLink bridge | Primary PT tier — best inference throughput for 7B–70B models |
| H200 NVL | 141 GB HBM3e | Hopper | PCIe + NVLink bridge | Long-context PT; very large models; highest memory in fleet |
| H100 HBM3 | 80 GB HBM3 | Hopper | PCIe | High-throughput PT serving |
...
H100 NVL and H200 NVL are PCIe form-factor cards with NVLink bridging between pairs. They are **not** the SXM/DGX form factor with NVSwitch (full all-to-all GPU interconnect at 900 GB/s). This matters for multi-GPU tensor parallelism on 70B+ models. Multi-node PT requires InfiniBand fabric — infrastructure team must confirm whether NDR InfiniBand is present before designing PT for models that span multiple nodes.
...
```
KV cache per token ≈ 2 × num_layers × num_heads × head_dim × dtype_bytes

Example — LLaMA-3 70B, BF16:
  = 2 × 80 × 64 × 128 × 2 = 2.62 MB per token

For a 4,096-token context:
  KV cache per active request = 4,096 × 2.62 MB = 10.7 GB

Concurrent requests on a single H100 NVL (80 GB, 85% usable = 68 GB):
  68 GB ÷ 10.7 GB = ~6 concurrent requests maximum
```

**PT implication:** The TPM a customer can buy is bounded not by GPU compute but by KV cache. A customer requesting long-context PT (128k token contexts) gets fewer concurrent request slots per GPU than a customer running short-context workloads. The PT sizing model must account for context length, not just request rate.
...
|---|---|---|
| Control plane | **KServe** (CNCF-graduated, OpenShift AI default) | Confirmed primary |
| LLM serving engine | **vLLM** | In production |
| Non-LLM serving | **Triton Inference Server** (under KServe) | In production |
| 70B+ / MoE models | **llm-d** (CNCF, March 2026) | Phase 4; pilot in progress |
| Fractional GPU | **MIG** via NVIDIA GPU Operator | Confirmed operational in A100 environment |
| Request routing | **Gateway API Inference Extension** (GA Feb 2026) | Phase 1 deployment target |
...
Evaluated and deliberately deferred: NVIDIA Dynamo (requires RDMA fabric, not available), NVIDIA Runcai (commercial GPU orchestration — Phase 5 build-vs-buy decision), Ray Serve (best-fit for fine-grained fractional GPU — deferred due to ops burden).
...
PT is enforced through dedicated serving pools — separate from the shared on-demand pool:
...
│
└── On-Demand Pool (shared)
    └── KServe InferenceService: model-X, shared
        └── vLLM pod(s) serving all on-demand requests
        └── HPA: scales dynamically with traffic
```

PT replicas have fixed replica counts (`minReplicas == maxReplicas == N`). They hold their GPU allocation permanently for the duration of the reservation. On-demand replicas scale dynamically based on traffic.

### 3.2 KServe InferenceService as the PT Unit of Enforcement

Each PT reservation maps to a KServe InferenceService with specific parameters:
...
PT pods must be physically isolated from on-demand pods to prevent noisy-neighbour effects on the same GPU node:
...
Without physical isolation, a bursty on-demand workload on the same GPU node as a PT workload can cause memory pressure, thermal throttling, or NVLink bandwidth contention — all of which degrade PT TTFT. Physical isolation is the only reliable guarantee.
...
| **`--gpu-memory-utilization`** | Controls what fraction of GPU memory vLLM claims at startup. Set to 0.90 for PT pools (full KV cache headroom); can be lower for on-demand pools. |
| **Priority queues** | vLLM supports request priority. If PT and on-demand share a pool (Phase 2 soft isolation), PT requests get priority over on-demand. |
  ...
  - coveredResources: ["nvidia.com/gpu"]
    ...
    - name: h100-nvl
      ...
      - name: "nvidia.com/gpu"
        ...
        borrowingLimit: 0   # Cannot borrow from on-demand pool
---
...
KV cache transfer between prefill and decode nodes requires fast inter-node interconnect:
- **Within a DGX H100 (NVSwitch):** 900 GB/s — excellent for llm-d
- **H100 NVL (NVLink bridge, PCIe form factor):** ~600 GB/s pairwise — functional within a node
- **Multi-node via InfiniBand NDR 400G:** ~50 GB/s effective — feasible but adds latency per token

**Infrastructure team must confirm:** Do our H100 NVL / H200 NVL nodes have InfiniBand connectivity, and at what speed? This determines whether multi-node llm-d PT is viable and what TTFT degradation to expect from KV transfer latency.
...
| Large | Full H100 NVL | 80 GB | 34B–70B LLMs on H100 | 1 per card |
| XL | Full H200 NVL | 141 GB | 70B LLMs at long context | 1 per card |
...
The Gateway API Inference Extension reached GA in February 2026. This is the request-routing layer for PT traffic.
...
Gateway API HTTPRoute
     ...
     │
     └── X-Tenant: on-demand (or no PT header)
             ↓
         InferencePool: ondemand-llama3-70b     ← routes to shared pool
             ↓
         vLLM on-demand replicas (dynamic)
```
