...
| Isolation model | Phase 1: physical node isolation (taints + affinity) for simplicity. Phase 2: logical isolation via llm-d flow control (priority bands + reserved capacity + per-tenant fairness). | Vertex uses logical isolation (scheduling priority, not hardware exclusivity). llm-d flow control benchmarks show Realtime p95 TTFT stays at ~500ms while lower-priority work absorbs the wait on shared GPUs — validating logical isolation for PT. Phase 2 logical isolation matches Vertex's approach with better fleet utilisation than Phase 1 physical isolation. |
