...
|---|---|---|---|
| **Phase 1** | Guaranteed TPM on H100 NVL or H200 NVL; dedicated node pool; TTFT SLA; fixed committed price | `LLMInferenceService` (KServe v0.17) with llm-d EPP auto-provisioning InferencePool + HTTPRoute; vLLM with per-request metrics; DCGM; node taints + ResourceQuota | PT Reservation CRD + Operator; PT Auth Service; Sizing Calculator; Utilisation Dashboard; Billing Pipeline |
| **Phase 2** | Request-level TPM enforcement; spillover to shared pool; finer-grained metering | Envoy AI Gateway `BackendTrafficPolicy` with `limit.fromMetadata` for token-based rate limiting; `InferenceObjective` (alpha) for PT priority scheduling | Spillover controller; per-tenant TPM budget injection via auth service; chargeback reconciliation |
