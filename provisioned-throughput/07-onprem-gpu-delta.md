...
|---|---|---|---|---|
| H100 NVL | 94 GB HBM3 per GPU (dual-GPU card, 188 GB) | PCIe + NVLink bridge | NVLink 600 GB/s between pairs | Primary PT tier — 7B–70B models |
| H200 NVL | 141 GB HBM3e | PCIe + 2/4-way NVLink bridge | NVLink 900 GB/s per GPU | Premium PT tier — 70B long-context, large model |
| H100 SXM | 80 GB HBM3 | SXM (DGX/HGX baseboard) | NVSwitch 900 GB/s all-to-all | PT serving; confirmed best utilisation in fleet |
...
**H100 NVL ≠ H100 SXM (DGX H100).** The NVL suffix indicates a dual-GPU PCIe card with NVLink bridges — not NVSwitch, which provides full all-to-all GPU connectivity at 900 GB/s. Note: H200 NVL supports 2-way or 4-way NVLink at 900 GB/s per GPU — significantly better interconnect than H100 NVL.

| Capability | H100 SXM (DGX, NVSwitch) | H100 NVL (PCIe, NVLink bridge) | H200 NVL (PCIe, 2/4-way NVLink) |
|---|---|---|---|
| GPU memory | 80 GB HBM3 | 94 GB HBM3 per GPU | 141 GB HBM3e |
| GPU-to-GPU bandwidth | 900 GB/s (NVLink 4.0, all-to-all) | ~600 GB/s (pairwise NVLink only) | 900 GB/s per GPU (2-way or 4-way bridge) |
| Tensor parallelism across all 8 GPUs | Full — NVSwitch enables any-to-any | Limited — efficient only across linked pairs | Better — 4-way NVLink groups of 4 GPUs |
| llm-d intra-node KV transfer | Excellent | Adequate but not optimal | Good — 900 GB/s within bridge groups |

**H100 NVL topology is confirmed** as PCIe + NVLink bridge (a defined NVIDIA product SKU). The open question is whether InfiniBand or RoCE RDMA is available for **cross-node** KV transfer, which is required for Phase 4 llm-d PT.
...
| NVIDIA changes AI Enterprise or Run:ai licensing terms mid-contract | Low | Prefer open-source stack (vLLM, KServe, DCGM community edition) where possible. Avoid NVIDIA AI Enterprise or Run:ai dependency for core PT functionality. |
