...
const phases = [
  ...
  },
  {
    ...
    timeline: 'Months 4-6',
    delivers: [
      'Logical isolation via llm-d flow control (priority bands + reserved capacity)',
      'Per-tenant fairness on shared GPU pools',
      'Priority spillover: overflow dispatches at Standard priority',
      'Batch eviction: reclaim capacity from lower-priority work in vLLM',
    ...
    builds: [
      ...
      'Flow control configuration per PT reservation',
    ],
    upstream: [
      'llm-d flow control (priority bands, dispatch gate, batch eviction) — benchmarked on H100',
      'InferenceObjective for priority band mapping',
      'Envoy AI Gateway BackendTrafficPolicy',
      'vLLM per-request metrics',
    ],
    openQuestions: [
      'Flow control configuration tuning for our fleet (maxConcurrency, reserved capacity %)',
      'Envoy AI Gateway tagged release for air-gap (TF-9)',
      'Fleet utilisation improvement from shared pools vs dedicated nodes',
    ],
  },
  {
    id: 3,
